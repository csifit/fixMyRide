    ...
    <main className={emergency ? "app emergency-theme" : "app"}>
      <aside className="sidebar">
        ...
        <div className="sidebar-foot">
          ...
          <div className="signed-in"><div className="avatar small">{profile.initials}</div><div><b>{profile.name}</b><span>{t.signedIn}</span></div><button onClick={() => notify("Demo session remains on this device")} aria-label={t.signOut}>↗</button></div>
        </div>
      </aside>
            ...
            <section className="critical-card">
              ...
              <div className="critical-grid">
                <div><span>{t.blood}</span><strong className="blood">{profile.blood}</strong></div>
                  <div><span>{t.allergies}</span><div className="tags">{profile.allergies.length ? profile.allergies.map(a => <strong key={a}>{a}</strong>) : <strong>None recorded</strong>}</div></div>
            ...
            <div className="info-grid">
              <article className="card">
                ...
                <dl>
                  ...
                  <div><dt>{t.donor}</dt><dd><span className="yes">{profile.donor}</span></dd></div>
                </dl>
              </article>
              <article className="card">
                <div className="section-title"><MiniIcon>✚</MiniIcon><h2>{t.medications}</h2>{doctorMode && <button className="add-btn">+ Add</button>}</div>
                <div className="med-list">{profile.medications.length ? profile.medications.map(m => <div className="med" key={m.name}><span className="pill">◆</span><div><strong>{m.name}</strong><p>{[m.dose, m.timing].filter(Boolean).join(" · ")}</p></div></div>) : <p className="empty-state">No medications recorded</p>}</div>
              </article>
              <article className="card">
                <div className="section-title"><MiniIcon>⌁</MiniIcon><h2>{t.conditions}</h2></div>
                <div className="detail-block"><span>{t.chronic}</span><div className="tags calm">{profile.chronic.length ? profile.chronic.map(c => <strong key={c}>{c}</strong>) : <strong>None recorded</strong>}</div></div>
                <div className="detail-block"><span>{t.surgeries}</span><strong>{profile.surgeries.join(", ") || "None recorded"}</strong></div>
              </article>
              <article className="card">
                <div className="section-title"><MiniIcon>◇</MiniIcon><h2>{t.devices}</h2></div>
                <p className="empty-state"><span>✓</span>{profile.implants.join(", ") || "No implants recorded"}</p>
              </article>
              <article className="card wide">
                <div className="section-title"><MiniIcon>♧</MiniIcon><h2>{t.contacts}</h2></div>
                <div className="contact-grid">
                  <div><span className="contact-avatar">EC</span><div><small>{t.emergency}</small><strong>{profile.emergency}</strong>{profile.emergencyPhone && <a href={`tel:${profile.emergencyPhone}`}>{profile.emergencyPhone}</a>}</div></div>
                  <div><span className="contact-avatar doctor">DR</span><div><small>{t.doctor}</small><strong>{profile.doctor}</strong>{profile.doctorPhone && <a href={`tel:${profile.doctorPhone}`}>{profile.doctorPhone}</a>}</div></div>
                </div>
              </article>
            </div>
            <p className="update-note">✓ {t.lastUpdated}: {profile.lastUpdated} · {profile.doctor}</p>
          </div>
        )}
        ...
        {section === "access" && (
          <div className="page narrow">
            ...
            <article className="timeline">
              {data.accessHistory.map((event) => <div key={event.id}><span className="timeline-icon">{event.initials}</span><div><b>{event.actor}</b><p>{event.context}</p><small>{event.occurredAt}</small></div><span className="access-type">{event.action}</span></div>)}
              {!data.accessHistory.length && <div><span className="timeline-icon">—</span><div><b>No access events</b><p>Authorized profile activity will appear here.</p></div></div>}
            </article>
          </div>
        )}
      </section>
      ...
      {doctorModal && <div className="modal-backdrop" onMouseDown={() => setDoctorModal(false)}>
        <div className="modal" role="dialog" aria-modal="true" aria-labelledby="doctor-title" onMouseDown={e => e.stopPropagation()}>
          ...
          <p>Open the clinical workspace to review patient profiles and access requests in this prototype.</p>
          <button className="verify-btn" onClick={() => { window.location.href = "/doctor"; }}>Continue to clinical portal</button>
          <small>Prototype only · Authentication and persistence are not implemented yet.</small>
        </div>
      </div>}
