...
function SupportTickets({ data, language, t }: { data: AdminDashboardData; language: Language; t: T }) {
  return <section className="admin-support-queue">
    ...
    <div>{data.supportTickets.map((ticket) => {
      ...
      return <details className="admin-support-ticket" key={ticket.id}>
        <summary>
          <span className="admin-support-reference"><strong>{ticket.reference}</strong><small>{t(`adminSupport.type.${ticket.ticketType}` as TranslationKey)}</small></span>
...
function Content({ section, data, language, t }: { section: AdminSection; data: AdminDashboardData; language: Language; t: T }) {
  ...
  return <><article className="admin-panel"><h2>{t("automotiveAdmin.securityTitle")}</h2><p>{t("automotiveAdmin.securityDescription")}</p><dl><div><dt>{t("automotiveAdmin.role")}</dt><dd>{data.administrator.role}</dd></div><div><dt>{t("automotiveAdmin.session")}</dt><dd>AAL2</dd></div><div><dt>{t("automotiveAdmin.generated")}</dt><dd>{formatDateTime(language, data.generatedAt)}</dd></div></dl></article><h2>{t("adminEmailBlocklist.registeredAccounts")}</h2><Table headers={[t("adminWorkflow.account"), t("adminWorkflow.accountType"), t("adminWorkflow.accountStatus"), t("adminWorkflow.operation")]} rows={data.workflow.accounts.map((account) => [<span key={account.authUserId}><strong>{account.displayName}</strong><small>{account.email}</small></span>, account.accountType, t(`adminWorkflow.status.${account.status}` as TranslationKey), <AccountStatusControl key={account.authUserId} account={account} t={t} />])} empty={t("automotiveAdmin.empty")} /><EmailBlocklistAdministration entries={data.workflow.emailBlocks} t={t} /></>;
}
