...
export default function WorkshopRepairsClient({ repairs, logoutAction }: { repairs: ManagedRepairWorkflow[]; logoutAction: () => Promise<void> }) {
  ...
  return <main className="settings-shell booking-inbox-shell"><header className="settings-topbar"><Link href="/workshop-manager">← {t("workspace.back")}</Link><strong>pitster</strong><select value={language} onChange={(event) => setLanguage(event.target.value as Language)} aria-label={t("a11y.languageSelector")}><option value="en">EN</option><option value="de">DE</option><option value="ro">RO</option><option value="hu">HU</option></select><form action={logoutAction}><button>{t("auth.logout")}</button></form></header><section className="settings-content booking-inbox-content"><p className="registration-kicker">{t("repairLifecycle.eyebrow")}</p><h1>{t("repairLifecycle.title")}</h1><p>{t("repairLifecycle.description")}</p><OperationalIntroduction title={t("operationalGuidance.howTitle")} description={t("phase5.repairs.intro")} outcomeLabel={t("operationalGuidance.whyLabel")} outcome={t("phase5.repairs.outcome")} stepsLabel={t("operationalGuidance.stepsLabel")} steps={[t("phase5.repairs.step1"), t("phase5.repairs.step2"), t("phase5.repairs.step3")]} /><div className="booking-inbox-toolbar"><strong>{visible.length} {t("repairLifecycle.visible")}</strong><label>{t("workshopBookings.filter")}<select value={filter} onChange={(event) => setFilter(event.target.value)}><option value="active">{t("repairLifecycle.filter.active")}</option><option value="completed">{t("workshopBookings.status.completed")}</option><option value="no_show">{t("workshopBookings.status.no_show")}</option><option value="all">{t("workshopBookings.filter.all")}</option></select><FieldHelp>{t("phase5.repairs.filterHelp")}</FieldHelp></label></div><div className="booking-inbox-list">{visible.map((repair) => <RepairCard key={repair.id} repair={repair} language={language} t={t} />)}{!visible.length && <OperationalEmptyState mark="R" title={t("repairLifecycle.emptyTitle")} description={t("phase5.repairs.emptyDescription")} action={<Link href="/workshop-manager/requests">{t("phase5.repairs.openRequests")}</Link>} />}</div></section></main>;
}
