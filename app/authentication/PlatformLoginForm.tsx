...
export default function PlatformLoginForm({ portal, configured }: { portal: "customer" | "workshop_manager" | "service_organisation"; configured: boolean }) {
  ...
  return <main className={`auth-shell ${ready ? "" : "i18n-pending"}`}><section className="auth-card">
    ...
    {!configured ? <p className="auth-error">{t("auth.configuration.description")}</p> : <form action={action}><input type="hidden" name="portal" value={portal} /><label>{t("auth.login.email")}<input name="email" type="email" required autoComplete="email" /></label><label>{t("auth.login.password")}<input name="password" type="password" required minLength={8} autoComplete="current-password" /></label><Link className="auth-forgot-link" href={`/forgot-password?portal=${portal}`}>{t("passwordRecovery.forgotLink")}</Link>{state.error && <p className="auth-error" role="alert">{t(state.error === "configuration" ? "auth.configuration.description" : `auth.login.${state.error}` as TranslationKey)}</p>}<button disabled={pending}>{t(pending ? "auth.login.submitting" : "auth.login.submit")}</button></form>}
    {portal === "customer" && <small>{t("platformLogin.noAccount")} <Link href="/register/customer">{t("platformLogin.register")}</Link></small>}
    {portal === "service_organisation" && <small>{t("platformLogin.noAccount")} <Link href="/register/workshop-manager">{t("platformLogin.register")}</Link></small>}
    {portal === "workshop_manager" && <small>{t("platformLogin.invitationOnly")}</small>}
  </section></main>;
}
